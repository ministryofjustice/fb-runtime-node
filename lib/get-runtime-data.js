...
const propagateNamespace = require('./propagate-namespace')
...
const getRuntimeData = (sourceObjs, schemas) => {
  return loadJSON.load(sourceObjs)
    .then(sourceInstances => {
      ...
      const namespaceInstances = propagateNamespace.propagate(stepsInstances, schemas)
      const showInstances = propagateShow.propagate(namespaceInstances)
