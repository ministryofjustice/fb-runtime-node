...
test('When propagating a repeatable instance without a namespace', function (t) {
  const input = {
    repeatableA: {
      _id: 'repeatableA',
      _type: 'page.form',
  ...
  try {
    propagate(input)
    t.notOk('This test should have thrown an error')
  } catch (e) {
    t.equal(e.name, 'FBPageRepeatableNamespaceError', 'it should throw an error')
    t.equal(e.message, 'repeatableA is repeatable but has no namespace', 'it should report the id of the page without the namespace as the error message')
    t.deepEqual(e.data, {_id: 'repeatableA', instances: {repeatableA: {_id: 'repeatableA', _type: 'page.form', repeatable: true, url: '/repeatableA', $FALLBACKurl: true}}}, 'it should return the instances processed so far as the data property of the error object')
  }
  t.end()
})

test('When a single step is missing', t => {
  const badData = getTestData('input-single-missing-step')

  try {
    propagate(badData)
  } catch (e) {
    t.equal(e.name, 'FBPageMissingError', 'it should return a FBPageMissingError')
    t.equal(e.data.step, 'stepA', 'it should return the missing step')
  }

  t.end()
})

test('When there are both present and missing steps', t => {
  ...
  try {
    ...
    t.equal(e.name, 'FBPageMissingError', 'it should return a FBPageMissingError')
    t.equal(e.data.step, 'stepX', 'it should return the missing step')
  }
