...
const getJsonSrc = sourceObj => {
  ...
  let jsonPaths
  try {
    jsonPaths = fs.readdirSync(sourceObj.path)
  } catch (e) {
    return Promise.reject(e)
  }
  jsonPaths = jsonPaths.filter(jsonPath => jsonPath.endsWith('.json'))
    .map(jsonPath => path.join(sourceObj.path, jsonPath))
 ...
 * @param {string} sourceObjs[].path path to load json from
 * @param {string} sourceObjs[].source source as key to store return loaded instances against
