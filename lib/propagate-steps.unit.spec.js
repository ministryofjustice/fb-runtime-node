...
test('When adding _parent properties to step instances', function (t) {
  ...
  t.equal(instances.topA.namePrefix, 'modelA')
  t.deepEqual(instances.topA.$models, ['modelA'])
  t.equal(instances.topA.url, '/top-a')
  t.equal(instances.stepA.namePrefix, 'modelA')
  t.deepEqual(instances.stepA.$models, ['modelA'])
  t.equal(instances.stepA.url, '/top-a/step-a')
  t.equal(instances.stepC.namePrefix, 'modelA.modelC[{modelC}]')
  t.deepEqual(instances.stepC.$models, ['modelA', 'modelC'])
  t.equal(instances.stepC.url, '/top-a/step-a/step-c/:modelC')
  ...
  t.deepEqual(instances.topB.$models, undefined)
  t.equal(instances.topB.url, '/topB')
  t.equal(instances.stepZ.namePrefix, 'modelZ')
  t.deepEqual(instances.stepZ.$models, ['modelZ'])
  t.equal(instances.stepZ.url, '/topB/step-z')
  t.equal(instances.topD.url, '/topD', 'it should use the page id if url is missing')
  t.equal(instances.topE.url, '/top-e/:modelE')
  t.equal(instances.topE.namePrefix, 'modelE[{modelE}]')
  t.deepEqual(instances.topE.$models, ['modelE'])
  t.equal(instances.stepF.url, '/top-e/:modelE/step-f')
  t.equal(instances.stepF.namePrefix, 'modelE[{modelE}].modelF')
  t.deepEqual(instances.stepF.$models, ['modelE', 'modelF'])
...
test('When a step is referenced by multiple parent pages', function (t) {
  ...
  try {
    ...
    const instances = propagate(badData)
