...
const propagate = instances => {
  ...
  let pageKeys = Object.keys(instances)
  pageKeys.forEach(key => {
    const instance = instances[key]
    if (instance._type.startsWith('page.')) {
      const isStep = pageKeys.filter(pageKey => {
        return instances[pageKey].steps && instances[pageKey].steps.includes(key)
      }).length
      if (!isStep) {
        FBLogger(`${key} is an entry point`)
        propagateStepConditions(instance)
      }
    }
  })

  // needs to start from entry points
  // const tosh = true
  // if (!tosh) {
  //   jp.query(instances, '$..[?(@.showSteps)]').forEach((showStepsInstance, i) => {
  //     FBLogger.log(i, showStepsInstance.showSteps)
  //     let stepsShow = deepClone(showStepsInstance.showSteps)
  //     if (showStepsInstance.show) {
  //       stepsShow = createAllConditions(showStepsInstance.show, stepsShow)
  //     }
  //     FBLogger.log(showStepsInstance.steps)
  //     showStepsInstance.steps.forEach(step => {
  //       const stepInstance = instances[step]
  //       FBLogger.log(stepInstance)
  //     })
  //   })
  // }
