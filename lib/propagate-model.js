 ...
 **/
const propagateInstance = (instance, schemas, namePrefix) => {
  ...
  if (typeInstructions) {
    ...
    subInstances.forEach(subInstance => {
      propagateInstance(subInstance, schemas, instance.namePrefix)
    })
  }
...
const propagate = (instances, schemas) => {
  ...
  Object.keys(schemas).forEach(type => {
    const categoryTypes = schemas[type].categories
    if (categoryTypes) {
